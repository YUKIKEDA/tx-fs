...
describe('tx-fs New Operations Tests', () => {
  ...
  describe('rename操作', () => {
    it('ファイルのリネームが正常に動作する', async () => {
      ...
      await expect(
        fs.access(path.join(testDir, 'original.txt')),
      ).rejects.toThrow();
    });
    ...
    it('ディレクトリのリネームが正常に動作する', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, 'original-dir', 'file.txt'),
        'content',
      );
      ...
      const newContent = await fs.readFile(
        path.join(testDir, 'renamed-dir', 'file.txt'),
        'utf-8',
      );
      ...
      await expect(
        fs.access(path.join(testDir, 'original-dir')),
      ).rejects.toThrow();
    });
    ...
    it('既存のターゲットがある場合のリネーム（上書き）を処理する', async () => {
      ...
      await expect(
        fs.access(path.join(testDir, 'source.txt')),
      ).rejects.toThrow();
    });
    ...
    it('トランザクション失敗時にリネーム操作をロールバックする', async () => {
      ...
      await expect(
        fs.access(path.join(testDir, 'renamed.txt')),
      ).rejects.toThrow();
    });
  ...
  describe('cp操作', () => {
    ...
    it('ディレクトリを再帰的にコピーする', async () => {
      ...
      await fs.mkdir(path.join(testDir, 'source-dir', 'subdir'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(testDir, 'source-dir', 'file1.txt'),
        'content1',
      );
      await fs.writeFile(
        path.join(testDir, 'source-dir', 'subdir', 'file2.txt'),
        'content2',
      );
      ...
      const content1 = await fs.readFile(
        path.join(testDir, 'copy-dir', 'file1.txt'),
        'utf-8',
      );
      const content2 = await fs.readFile(
        path.join(testDir, 'copy-dir', 'subdir', 'file2.txt'),
        'utf-8',
      );
  ...
  describe('snapshotDir操作', () => {
    it('ディレクトリのスナップショットを作成する', async () => {
      ...
      await fs.mkdir(path.join(testDir, 'test-dir', 'subdir'), {
        recursive: true,
      });
      await fs.writeFile(
      ...
      await fs.writeFile(
        path.join(testDir, 'test-dir', 'subdir', 'file2.txt'),
        'content2',
      );
      ...
      const newContent = await fs.readFile(
        path.join(testDir, 'test-dir', 'new-file.txt'),
        'utf-8',
      );
    ...
    it('ロールバックシナリオでスナップショットを処理する', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, 'test-dir', 'file.txt'),
        'original content',
      );
