...
`,
        ...
        // テストファイル
        await tx.writeFile(
          'testfiles/calculator-unit-test.js',
          `
...
`,
      ...
      await txManager.run(async (tx) => {
        ...
        const packageJson = JSON.parse(
          (await tx.readFile('package.json', 'utf-8')) as string,
        );
...
Permission is hereby granted, free of charge, to any person obtaining a copy...`,
      ...
      const buildInfo = JSON.parse(
        await fs.readFile(path.join(testDir, 'dist/build-info.json'), 'utf-8'),
      );
...
`,
      ...
      const testFileExists = await fs
        .access(path.join(testDir, 'test_main.py'))
        .then(() => true)
        .catch(() => false);
...
`,
      ...
      await txManager.run(async (tx) => {
        // CSVデータをJSONに変換
        const usersCSV = (await tx.readFile(
          'data/raw/users.csv',
          'utf-8',
        )) as string;
        ...
        // 注文データの変換
        const ordersCSV = (await tx.readFile(
          'data/raw/orders.csv',
          'utf-8',
        )) as string;
      ...
      await txManager.run(async (tx) => {
        ...
        const users = JSON.parse(
          (await tx.readFile('data/processed/users.json', 'utf-8')) as string,
        );
        const orders = JSON.parse(
          (await tx.readFile('data/processed/orders.json', 'utf-8')) as string,
        );
        ...
        const userOrderSummary = users.map((user) => {
          ...
          return {
            ...
            avg_order_value:
              userOrders.length > 0 ? totalAmount / userOrders.length : 0,
          };
        });

        await tx.writeFile(
          'data/output/user_summary.json',
          JSON.stringify(userOrderSummary, null, 2),
        );
        ...
        await tx.writeFile(
          'data/output/country_stats.json',
          JSON.stringify(Object.values(countryStats), null, 2),
        );
      ...
      const userSummary = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'data/output/user_summary.json'),
          'utf-8',
        ),
      );
      ...
      const countryStats = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'data/output/country_stats.json'),
          'utf-8',
        ),
      );
      ...
      const report = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'data/output/etl_report.json'),
          'utf-8',
        ),
      );
...
`,
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.writeFile(
          'logs/app.log',
          `${new Date().toISOString()} INFO Log rotation completed\n`,
        );
        ...
        await tx.writeFile(
          'logs/rotation.json',
          JSON.stringify(rotationInfo, null, 2),
        );
      });
      ...
      await txManager.run(async (tx) => {
        ...
        for (const compressedLog of compressedLogs) {
          await tx.writeFile(
            `logs/archive/${compressedLog}`,
            `[COMPRESSED] Original log content for ${compressedLog}`,
          );
        }
      ...
      // 検証
      const rotationInfo = JSON.parse(
        await fs.readFile(path.join(testDir, 'logs/rotation.json'), 'utf-8'),
      );
      expect(rotationInfo.archived_files).toHaveLength(3);

      const cleanupReport = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'logs/cleanup-report.json'),
          'utf-8',
        ),
      );
      expect(cleanupReport.compressed_files).toHaveLength(3);

      const newAppLog = await fs.readFile(
        path.join(testDir, 'logs/app.log'),
        'utf-8',
      );
      expect(newAppLog).toContain('Log rotation completed');

      const archiveExists = await fs
        .access(path.join(testDir, 'logs/archive'))
        .then(() => true)
        .catch(() => false);
  ...
  describe('Webアプリケーションデプロイメント用例', () => {
    it('Blue-Greenデプロイメントをシミュレートする', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        // 現在の本番アプリケーション
        await tx.writeFile(
          'production/green/app/index.html',
          `
<!DOCTYPE html>
<html>
...
`,
        ...
        await tx.writeFile(
          'production/green/config/app.json',
          JSON.stringify(
            {
              version: '1.0.0',
      ...
      await txManager.run(async (tx) => {
        ...
        // 新バージョンのアプリケーション
        await tx.writeFile(
          'production/blue/app/index.html',
          `
<!DOCTYPE html>
<html>
...
`,
        ...
        await tx.writeFile(
          'production/blue/config/app.json',
          JSON.stringify(
            {
              version: '2.0.0',
              environment: 'production',
        ...
        // デプロイメント情報
        await tx.writeFile(
          'production/blue/deployment.json',
          JSON.stringify(
            {
...
`,
          ...
          await tx.writeFile(
            'production/green/config/app.json',
            JSON.stringify(greenConfig, null, 2),
          );
        }
      });
      ...
      await txManager.run(async (tx) => {
        // デプロイメント成功の確認
        const active = (await tx.readFile(
          'production/active',
          'utf-8',
        )) as string;
        ...
        if (active.trim() === 'blue') {
          ...
          await tx.writeFile(
            'production/rollback-ready.flag',
            'green environment ready for rollback',
          );
        }
      });
      ...
      const active = await fs.readFile(
        path.join(testDir, 'production/active'),
        'utf-8',
      );
      ...
      const blueConfig = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'production/blue/config/app.json'),
          'utf-8',
        ),
      );
      ...
      const report = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'production/deployment-report.json'),
          'utf-8',
        ),
      );
      expect(report.deployment_successful).toBe(true);

      const rollbackReady = await fs
        .access(path.join(testDir, 'production/rollback-ready.flag'))
        .then(() => true)
        .catch(() => false);
...
`,
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.cp(
          'system/config/database.yaml',
          `system/backup/database-backup-${Date.now()}.yaml`,
        );
...
`,
      ...
      await txManager.run(async (tx) => {
        await tx.cp(
          'system/config/redis.yaml',
          `system/backup/redis-backup-${Date.now()}.yaml`,
        );
...
`,
      ...
      await txManager.run(async (tx) => {
        await tx.cp(
          'system/config/app.yaml',
          `system/backup/app-backup-${Date.now()}.yaml`,
        );
...
`,
      ...
      await txManager.run(async (tx) => {
        ...
        const configSummary = {
          ...
          new_features: ['real_time_chat', 'advanced_search', 'audit_logging'],
        ...
        await tx.writeFile(
          'system/config-update-summary.json',
...
`,
      ...
      const summary = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'system/config-update-summary.json'),
          'utf-8',
        ),
      );
      ...
      const appConfig = await fs.readFile(
        path.join(testDir, 'system/config/app.yaml'),
        'utf-8',
      );
      expect(appConfig).toContain('version: 2.0.0');
      expect(appConfig).toContain('real_time_chat: true');

      const changesLog = await fs.readFile(
        path.join(testDir, 'system/config/changes.log'),
        'utf-8',
      );
