  ...
  describe('複雑なワークフロー統合テスト', () => {
    it('プロジェクト構築ワークフロー（ビルドプロセス）をシミュレートする', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.rename(
          'build/src/components/Button.ts',
          'build/src/components/Button.min.js',
        );
      });
      ...
      await txManager.run(async (tx) => {
        ...
        const packageJson = JSON.parse(
          (await tx.readFile('build/package.json', 'utf-8')) as string,
        );
      ...
      // 結果検証
      const buildComplete = await fs.readFile(
        path.join(testDir, 'build/.build-complete'),
        'utf-8',
      );
      expect(buildComplete).toBeTruthy();

      const packageJson = JSON.parse(
        await fs.readFile(path.join(testDir, 'build/package.json'), 'utf-8'),
      );
      expect(packageJson.version).toBe('1.0.1');

      const mainExists = await fs
        .access(path.join(testDir, 'build/src/main.min.js'))
        .then(() => true)
        .catch(() => false);
      expect(mainExists).toBe(true);

      const utilsExists = await fs
        .access(path.join(testDir, 'build/src/utils'))
        .then(() => true)
        .catch(() => false);
    ...
    it('バックアップ・復元ワークフローをシミュレートする', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.cp('data', `backups/backup-${backupTimestamp}/data`, {
          recursive: true,
        });
        ...
        await tx.writeFile(
          `backups/backup-${backupTimestamp}/backup-info.json`,
      ...
      await txManager.run(async (tx) => {
        ...
        // バックアップから復元
        await tx.cp(`backups/backup-${backupTimestamp}/data`, 'data', {
          recursive: true,
        });
        ...
        await tx.writeFile(
          'restore.log',
          `Restored from backup-${backupTimestamp} at ${new Date().toISOString()}`,
        );
      });

      // 結果検証
      const user1 = JSON.parse(
        await fs.readFile(path.join(testDir, 'data/users/user1.json'), 'utf-8'),
      );
      expect(user1.name).toBe('Alice'); // 更新前の状態に戻っている

      const appLogExists = await fs
        .access(path.join(testDir, 'data/logs/app.log'))
        .then(() => true)
        .catch(() => false);
      expect(appLogExists).toBe(true); // 削除されたファイルが復元されている

      const user3Exists = await fs
        .access(path.join(testDir, 'data/users/user3.json'))
        .then(() => true)
        .catch(() => false);
      expect(user3Exists).toBe(false); // 新しく追加されたファイルは存在しない

      const restoreLog = await fs.readFile(
        path.join(testDir, 'restore.log'),
        'utf-8',
      );
    ...
    it('データベース移行ワークフローをシミュレートする', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        // V1 スキーマのデータ
        await tx.writeFile(
          'database/v1/users/1.json',
          '{"id": 1, "username": "alice", "email": "alice@example.com"}',
        );
        await tx.writeFile(
          'database/v1/users/2.json',
          '{"id": 2, "username": "bob", "email": "bob@example.com"}',
        );
        await tx.writeFile(
          'database/v1/orders/1.json',
          '{"id": 1, "user_id": 1, "amount": 100}',
        );
        await tx.writeFile(
          'database/v1/schema.json',
          '{"version": 1, "tables": ["users", "orders"]}',
        );
      });
      ...
      await txManager.run(async (tx) => {
        // ユーザーデータの移行（スキーマ変更）
        const user1 = JSON.parse(
          (await tx.readFile('database/v1/users/1.json', 'utf-8')) as string,
        );
        const user2 = JSON.parse(
          (await tx.readFile('database/v1/users/2.json', 'utf-8')) as string,
        );
        ...
        await tx.writeFile(
          'database/v2/users/1.json',
          JSON.stringify(user1V2, null, 2),
        );
        await tx.writeFile(
          'database/v2/users/2.json',
          JSON.stringify(user2V2, null, 2),
        );
        ...
        await tx.writeFile(
          'database/v2/schema.json',
          JSON.stringify(v2Schema, null, 2),
        );

        // 移行監査ログ
        await tx.writeFile(
          'database/v2/audit/migration.log',
          'Migrated 2 users and 1 order from v1 to v2',
        );
      });
      ...
      const currentSchema = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'database/current/schema.json'),
          'utf-8',
        ),
      );
      expect(currentSchema.version).toBe(2);

      const migratedUser = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'database/current/users/1.json'),
          'utf-8',
        ),
      );
      ...
      const version = await fs.readFile(
        path.join(testDir, 'database/VERSION'),
        'utf-8',
      );
      ...
      const userOperations = [
        ...
        txManager.run(async (tx) => {
          ...
          const config = JSON.parse(
            (await tx.readFile('project/config.json', 'utf-8')) as string,
          );
    ...
    it('継続的統合パイプラインをシミュレートする', async () => {
      ...
      const pipelineSteps = [
        ...
        txManager.run(async (tx) => {
          ...
          await tx.writeFile(
            'reports/test.xml',
            '<?xml version="1.0"?><testsuite tests="1" failures="0"></testsuite>',
          );
        }),
        ...
        txManager.run(async (tx) => {
          ...
          await tx.writeFile(
            'docs/README.html',
            '<html><body><h1>Project Documentation</h1></body></html>',
          );
        }),
      ];
      ...
      const pipelineReport = JSON.parse(
        await fs.readFile(path.join(testDir, 'reports/pipeline.json'), 'utf-8'),
      );
      expect(pipelineReport.status).toBe('success');

      const artifactsExist = await fs
        .access(path.join(testDir, 'artifacts'))
        .then(() => true)
        .catch(() => false);
      expect(artifactsExist).toBe(true);

      const docsExist = await fs
        .access(path.join(testDir, 'artifacts/docs/README.html'))
        .then(() => true)
        .catch(() => false);
    ...
    it('複雑なワークフロー中の部分失敗からリカバリーする', async () => {
      ...
      await expect(
        txManager.run(async (tx) => {
          ...
          // Step 3: 設定更新（成功）
          const config = JSON.parse(
            (await tx.readFile('workspace/config.json', 'utf-8')) as string,
          );
      ...
      // リカバリー後の状態確認
      const importantExists = await fs
        .access(path.join(testDir, 'workspace/important.txt'))
        .then(() => true)
        .catch(() => false);
      expect(importantExists).toBe(true);

      const newFileExists = await fs
        .access(path.join(testDir, 'workspace/new-file.txt'))
        .then(() => true)
        .catch(() => false);
      expect(newFileExists).toBe(false); // ロールバックされている

      const backupExists = await fs
        .access(path.join(testDir, 'backup'))
        .then(() => true)
        .catch(() => false);
      expect(backupExists).toBe(false); // ロールバックされている

      const config = JSON.parse(
        await fs.readFile(path.join(testDir, 'workspace/config.json'), 'utf-8'),
      );
    ...
    it('マルチステップ移行の途中失敗をハンドルする', async () => {
      ...
      await expect(
        txManager.run(async (tx) => {
          ...
          // Phase 2: データ変換（成功）
          const csvData = (await tx.readFile(
            'old-system/data/users.csv',
            'utf-8',
          )) as string;
          ...
          for (const line of lines) {
            if (line.trim()) {
              ...
              await tx.writeFile(
                `new-system/data/user-${id}.json`,
                JSON.stringify({ id: parseInt(id), name }),
              );
            }
          }
      ...
      // 失敗後の状態確認
      const oldSystemExists = await fs
        .access(path.join(testDir, 'old-system'))
        .then(() => true)
        .catch(() => false);
      expect(oldSystemExists).toBe(true); // 元データは保護されている

      const newSystemExists = await fs
        .access(path.join(testDir, 'new-system'))
        .then(() => true)
        .catch(() => false);
      ...
      const originalData = await fs.readFile(
        path.join(testDir, 'old-system/data/users.csv'),
        'utf-8',
      );
  ...
  describe('状態遷移とライフサイクル統合テスト', () => {
    it('アプリケーションライフサイクル全体をテストする', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.writeFile(
          'app/logs/app.log',
          'New log file started at ' + new Date().toISOString(),
        );
      ...
      // Verification
      const version = await fs.readFile(
        path.join(testDir, 'app/VERSION'),
        'utf-8',
      );
      expect(version).toBe('1.1.0');

      const maintenanceExists = await fs
        .access(path.join(testDir, 'app/.maintenance'))
        .then(() => true)
        .catch(() => false);
      expect(maintenanceExists).toBe(false);

      const backupExists = await fs
        .access(path.join(testDir, 'app/data/users.db.backup'))
        .then(() => true)
        .catch(() => false);
      expect(backupExists).toBe(true);

      const configBackupExists = await fs
        .access(path.join(testDir, 'app/config.backup'))
        .then(() => true)
        .catch(() => false);
    ...
    it('システム設定の段階的更新をテストする', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.writeFile(
          'system/config/security.conf',
          'firewall=enabled\nselinux=enforcing',
        );
        await tx.writeFile(
          'system/services/status.json',
          '{"web": "running", "db": "running"}',
        );
      });
      ...
      await txManager.run(async (tx) => {
        ...
        const services = JSON.parse(
          (await tx.readFile('system/services/status.json', 'utf-8')) as string,
        );
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.writeFile(
          ...
          'Security configuration updated at ' + new Date().toISOString(),
        );
      });
      ...
      await txManager.run(async (tx) => {
        ...
        await tx.writeFile(
          'system/optimization-report.txt',
          'System optimization completed successfully',
        );
      });

      // Final verification
      const networkConf = await fs.readFile(
        path.join(testDir, 'system/config/network.conf'),
        'utf-8',
      );
      expect(networkConf).toContain('ip=192.168.1.100');

      const securityConf = await fs.readFile(
        path.join(testDir, 'system/config/security.conf'),
        'utf-8',
      );
      ...
      const services = JSON.parse(
        await fs.readFile(
          path.join(testDir, 'system/services/status.json'),
          'utf-8',
        ),
      );
      ...
      const optimizationReport = await fs.readFile(
        path.join(testDir, 'system/optimization-report.txt'),
        'utf-8',
      );
