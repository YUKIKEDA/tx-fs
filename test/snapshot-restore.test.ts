...
describe('tx-fs スナップショット復元テスト', () => {
  ...
  describe('スナップショット復元を伴うrename', () => {
    ...
    it('上書きリネームがロールバックされた時に元のディレクトリを復元する', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, 'source-dir', 'file.txt'),
        'source content',
      );
      ...
      await fs.writeFile(
        path.join(testDir, 'target-dir', 'file.txt'),
        'original target content',
      );
      await fs.writeFile(
        path.join(testDir, 'target-dir', 'extra.txt'),
        'extra content',
      );
  ...
  describe('単一トランザクション内の複数スナップショット', () => {
    it('ロールバック時に複数の上書きファイルを復元する', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, 'target1.txt'),
        'original content 1',
      );
      await fs.writeFile(
        path.join(testDir, 'target2.txt'),
        'original content 2',
      );
      ...
      const target1Content = await fs.readFile(
        path.join(testDir, 'target1.txt'),
        'utf-8',
      );
      const target2Content = await fs.readFile(
        path.join(testDir, 'target2.txt'),
        'utf-8',
      );
      ...
      const source1Content = await fs.readFile(
        path.join(testDir, 'source1.txt'),
        'utf-8',
      );
      expect(source1Content).toBe('source content 1');

      // Check that source2 still exists (copy preserves source)
      const source2Content = await fs.readFile(
        path.join(testDir, 'source2.txt'),
        'utf-8',
      );
  ...
  describe('スナップショット復元のエッジケース', () => {
    ...
    it('スナップショットが作成されていない場合のロールバックを処理する', async () => {
      ...
      const targetExists = await fs
        .access(path.join(testDir, 'target.txt'))
        .then(() => true)
        .catch(() => false);
