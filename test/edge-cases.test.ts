...
describe('tx-fs エッジケース・エラーハンドリングテスト', () => {
  ...
  describe('パス関連のエッジケース', () => {
    ...
    it('特殊文字を含むファイル名を処理する', async () => {
      const specialChars = ['ファイル名.txt', 'file with spaces.txt', 'file-with-dashes.txt', 'file_with_underscores.txt'];
    ...
    it('深いディレクトリ構造を処理する', async () => {
      const deepPath = Array(10).fill(0).map((_, i) => `level${i}`).join('/') + '/deep-file.txt';
    ...
    it('ドット始まりのファイル名（隠しファイル）を処理する', async () => {
      ...
      const hiddenContent = await fs.readFile(path.join(testDir, '.hidden-file'), 'utf-8');
      const nestedContent = await fs.readFile(path.join(testDir, '.hidden-dir/.nested-hidden'), 'utf-8');
  ...
  describe('ファイルサイズとコンテンツのエッジケース', () => {
    ...
    it('大きなファイル（1MB）を処理する', async () => {
      ...
      const content = await fs.readFile(path.join(testDir, 'large.txt'), 'utf-8');
  ...
  describe('ディレクトリ操作のエッジケース', () => {
    ...
    it('空でないディレクトリの削除を処理する', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'non-empty-dir', 'file.txt'), 'content');
      ...
      await expect(fs.access(path.join(testDir, 'non-empty-dir'))).rejects.toThrow();
    });
  ...
  describe('競合状態とロック処理', () => {
    it('同じファイルへの複数書き込みを処理する', async () => {
      ...
      const content = await fs.readFile(path.join(testDir, 'multi-write.txt'), 'utf-8');
    ...
    it('同じディレクトリでの複数操作を処理する', async () => {
      ...
      const content1 = await fs.readFile(path.join(testDir, 'test-dir/file1.txt'), 'utf-8');
      const content2 = await fs.readFile(path.join(testDir, 'test-dir/file2.txt'), 'utf-8');
      const content3 = await fs.readFile(path.join(testDir, 'test-dir/subdir/file3.txt'), 'utf-8');
  ...
  describe('トランザクション境界のエッジケース', () => {
    it('未初期化マネージャーでの操作を拒否する', async () => {
      ...
      await expect(uninitializedManager.run(async (tx) => {
    ...
    it('ネストしたトランザクション操作を処理する', async () => {
      ...
      const outerContent = await fs.readFile(path.join(testDir, 'outer.txt'), 'utf-8');
      const innerContent = await fs.readFile(path.join(testDir, 'inner.txt'), 'utf-8');
      const nestedContent = await fs.readFile(path.join(testDir, 'nested-dir/nested-file.txt'), 'utf-8');
  ...
  describe('エラー回復とクリーンアップ', () => {
    it('部分的失敗後のクリーンアップを確認する', async () => {
      ...
      await expect(fs.access(path.join(testDir, 'file1.txt'))).rejects.toThrow();
