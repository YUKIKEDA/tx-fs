...
describe('tx-fs 堅牢性・障害シナリオテスト', () => {
  ...
  describe('リソース枯渇シナリオ', () => {
      ...
      try {
        for (let i = 0; i < iterations; i++) {
          await txManager.run(async (tx) => {
            ...
            console.log(`Memory usage after file ${i}: ${Math.round(memUsage.heapUsed / 1024 / 1024)}MB`);
        ...
        for (let i = 0; i < iterations; i++) {
          const exists = await fs.access(path.join(testDir, `large-file-${i}.txt`)).then(() => true).catch(() => false);
    ...
    it('ファイルハンドル枯渇をテストする', async () => {
      ...
      try {
        ...
        console.log(`File handle test: ${successes.length} succeeded, ${failures.length} failed`);
    ...
    it('並行トランザクション数の制限をテストする', async () => {
      ...
      for (let i = 0; i < concurrentCount; i++) {
        const promise = txManager.run(async (tx) => {
          await new Promise(resolve => setTimeout(resolve, 100 + Math.random() * 100)); // ランダム遅延
          await tx.writeFile(`concurrent-${i}.txt`, `Concurrent operation ${i}`);
      ...
      console.log(`Concurrent transactions: ${successes} succeeded, ${failures} failed`);
  ...
  describe('ファイルシステム障害シナリオ', () => {
    it('権限エラーに対する堅牢性をテストする', async () => {
      ...
      try {
        ...
        const content = await fs.readFile(path.join(testDir, 'normal-file.txt'), 'utf-8');
        expect(content).toBe('normal content');
        
      } catch (chmodError) {
        console.log('Permission test skipped due to chmod failure:', chmodError);
      } finally {
        try {
    ...
    it('パス長制限への対処をテストする', async () => {
      ...
      try {
        await txManager.run(async (tx) => {
          ...
          // 深い階層にファイルを作成
          await tx.writeFile(path.join(currentPath, 'deep-file.txt'), 'content at maximum depth');
        });
        
        // 作成されたファイルの確認
        const content = await fs.readFile(path.join(testDir, currentPath, 'deep-file.txt'), 'utf-8');
    ...
    it('ディスク容量制限のシミュレーションをテストする', async () => {
      ...
      for (const size of testSizes) {
        try {
          ...
          const stats = await fs.stat(path.join(testDir, `large-${size}-bytes.txt`));
          expect(stats.size).toBe(size);
          
        } catch (error: any) {
          // ディスク容量不足やメモリ制限は受け入れ可能
          console.log(`Disk space test failed at ${size} bytes:`, error.message);
          expect(error).toBeInstanceOf(Error);
  ...
  describe('データ破損と回復シナリオ', () => {
      ...
      try {
        ...
        if (journalFiles.length > 0) {
          for (const journalFile of journalFiles) {
            ...
            await fs.writeFile(path.join(journalDir, journalFile), 'CORRUPTED_DATA_NOT_JSON');
          }
        }
      } catch (e) {
    ...
    it('ステージングディレクトリ破損からの回復をテストする', async () => {
      ...
      try {
        txPromise = txManager.run(async (tx) => {
          ...
          if (stagingDirs.length > 0) {
            ...
            // ステージング内のファイルを削除/破損
            try {
              const stagedFiles = await fs.readdir(stagingDir, { recursive: true });
        ...
        await expect(txPromise).rejects.toThrow('Simulated failure after staging corruption');
      } catch (error) {
        // 予期されるエラー
      }
      ...
      const content = await fs.readFile(path.join(testDir, 'post-corruption-test.txt'), 'utf-8');
      expect(content).toBe('system stable');
      
      // 破損したファイルが本体ディレクトリに作成されていないことを確認
      const corruptedExists = await fs.access(path.join(testDir, 'staged-file.txt')).then(() => true).catch(() => false);
    ...
    it('部分的書き込み失敗からの回復をテストする', async () => {
      ...
      for (let i = 0; i < fileCount; i++) {
        const exists = await fs.access(path.join(testDir, `batch-file-${i}.txt`)).then(() => true).catch(() => false);
      ...
      const content = await fs.readFile(path.join(testDir, 'after-partial-failure.txt'), 'utf-8');
  ...
  describe('競合状態とタイミング攻撃', () => {
    it('トランザクション実行中の外部変更を検出する', async () => {
      // 外部からファイルを作成
      await fs.writeFile(path.join(testDir, 'external-file.txt'), 'external content');
      ...
      const content = await fs.readFile(path.join(testDir, 'external-file.txt'), 'utf-8');
    ...
    it('高頻度の並行アクセスでのデータ整合性をテストする', async () => {
      ...
      for (let i = 0; i < operationCount; i++) {
        const promise = txManager.run(async (tx) => {
          try {
            const currentValue = parseInt(await tx.readFile('counter.txt', 'utf-8') as string);
            await new Promise(resolve => setTimeout(resolve, Math.random() * 10)); // ランダム遅延
      ...
      console.log(`Concurrent access test: ${successes.length} succeeded, ${failures.length} failed`);
      
      // 最終的なカウンター値を確認
      const finalValue = parseInt(await fs.readFile(path.join(testDir, 'counter.txt'), 'utf-8'));
    ...
    it('システムリソース競合での優雅な劣化をテストする', async () => {
      ...
      console.log(`Heavy I/O test: ${successes.length} succeeded, ${failures.length} failed`);
    ...
    it('ファイル名長制限での動作をテストする', async () => {
      ...
      for (const length of nameLengths) {
        ...
        try {
          ...
          // 成功した場合は内容を確認
          const content = await fs.readFile(path.join(testDir, longName), 'utf-8');
    ...
    it('ファイルシステム固有の制限をテストする', async () => {
      ...
      for (const name of reservedNames) {
        try {
          ...
          // 成功した場合（Unix系システム）
          const exists = await fs.access(path.join(testDir, name)).then(() => true).catch(() => false);
          if (exists) {
    ...
    it('エンコーディング境界での動作をテストする', async () => {
      ...
      for (let i = 0; i < testStrings.length; i++) {
        ...
        try {
          ...
          const readContent = await fs.readFile(path.join(testDir, filename), 'utf-8');
  ...
  describe('復旧とクリーンアップの堅牢性', () => {
    it('不完全なクリーンアップ状態からの復旧をテストする', async () => {
      // 意図的に不完全な状態を作成
      const txDir = path.join(testDir, '.tx');
      await fs.mkdir(path.join(txDir, 'staging', 'orphaned-tx'), { recursive: true });
      await fs.writeFile(path.join(txDir, 'staging', 'orphaned-tx', 'orphaned-file.txt'), 'orphaned content');
      
      // 孤立したジャーナルファイルも作成
      await fs.mkdir(path.join(txDir, 'journal'), { recursive: true });
      await fs.writeFile(path.join(txDir, 'journal', 'orphaned-tx.json'), '{"id": "orphaned-tx", "status": "UNKNOWN"}');
      ...
      const content = await fs.readFile(path.join(testDir, 'cleanup-test.txt'), 'utf-8');
      expect(content).toBe('cleanup successful');
      
      // 孤立したファイルがクリーンアップされているかチェック
      const orphanedExists = await fs.access(path.join(txDir, 'staging', 'orphaned-tx')).then(() => true).catch(() => false);
    ...
    it('メタデータディレクトリの再構築をテストする', async () => {
      ...
      const content = await fs.readFile(path.join(testDir, 'rebuilt/test.txt'), 'utf-8');
      expect(content).toBe('rebuild successful');
      
      // メタデータディレクトリが再構築されていることを確認
      const txDirExists = await fs.access(txDir).then(() => true).catch(() => false);
    ...
    it('異常終了後の自動回復をテストする', async () => {
      ...
      // ベースラインファイルは残っているはず
      const baselineContent = await fs.readFile(path.join(testDir, 'baseline.txt'), 'utf-8');
      expect(baselineContent).toBe('baseline content');
      
      // 不完全なファイルは存在しないはず
      const incompleteExists = await fs.access(path.join(testDir, 'incomplete.txt')).then(() => true).catch(() => false);
      ...
      const recoveryContent = await fs.readFile(path.join(testDir, 'post-recovery.txt'), 'utf-8');
