...
describe('tx-fs パフォーマンス・ストレステスト', () => {
  ...
  describe('大量ファイル操作', () => {
    it('100個のファイル作成を処理する', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        const content = await fs.readFile(
          path.join(testDir, `file_${i.toString().padStart(3, '0')}.txt`),
          'utf-8',
        );
    ...
    it('大量ファイルのコピー操作を処理する', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        const content = await fs.readFile(
          path.join(testDir, `dest/file_${i}.txt`),
          'utf-8',
        );
    ...
    it('階層ディレクトリ構造の作成を処理する', async () => {
      ...
      const deepFile = path.join(
        testDir,
        'level_0/level_1/level_2/level_3/level_4/file_0.txt',
      );
  ...
  describe('大容量ファイル処理', () => {
    it('5MBファイルの作成と読み取りを処理する', async () => {
      ...
      const readContent = await fs.readFile(
        path.join(testDir, 'large_file.txt'),
        'utf-8',
      );
    ...
    it('複数の大容量ファイルコピーを処理する', async () => {
      ...
      await txManager.run(async (tx) => {
        ...
        for (let i = 0; i < 3; i++) {
          await tx.cp(
            `large_source/large_${i}.txt`,
            `large_dest/large_${i}.txt`,
          );
        }
      });
      ...
      for (let i = 0; i < 3; i++) {
        const copiedContent = await fs.readFile(
          path.join(testDir, `large_dest/large_${i}.txt`),
          'utf-8',
        );
      ...
      for (let dirIndex = 0; dirIndex < 5; dirIndex++) {
        for (let fileIndex = 0; fileIndex < 10; fileIndex++) {
          const filePath = path.join(
            testDir,
            `concurrent_dir_${dirIndex}/file_${fileIndex}.txt`,
          );
  ...
  describe('メモリ効率テスト', () => {
    ...
    it('大容量ファイル処理でのメモリ効率を確認する', async () => {
      ...
      console.log(
        `大容量ファイル処理メモリ増加: ${Math.round(memoryIncrease / 1024 / 1024)}MB`,
      );
  ...
  describe('スケーラビリティテスト', () => {
    it('ディレクトリ内の大量ファイルを処理する', async () => {
      ...
      console.log(
        `1ファイルあたり平均時間: ${(duration / fileCount).toFixed(2)}ms`,
      );
      ...
      for (const index of randomIndices) {
        const content = await fs.readFile(
          path.join(
            testDir,
            `scalability_test/file_${index.toString().padStart(4, '0')}.txt`,
          ),
          'utf-8',
        );
