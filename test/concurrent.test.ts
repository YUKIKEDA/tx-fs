...
describe('tx-fs Concurrent Tests', () => {
  ...
  it('成功と失敗が混在するシナリオを処理できる', async () => {
    ...
    // 成功したファイルは存在する
    const success1 = await fs.readFile(
      path.join(BASE_DIR, 'success-1.txt'),
      'utf-8',
    );
    expect(success1).toBe('success 1');

    const success2 = await fs.readFile(
      path.join(BASE_DIR, 'success-2.txt'),
      'utf-8',
    );
    ...
    await expect(
      fs.access(path.join(BASE_DIR, 'failure-1.txt')),
    ).rejects.toThrow();

    // 共有ファイルは元の内容のまま
    const sharedContent = await fs.readFile(
      path.join(BASE_DIR, 'shared.txt'),
      'utf-8',
    );
